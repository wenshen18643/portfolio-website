...
The fictional MT5 account 880042 is automatically assigned. Never ask the visitor for an MT5 login/account number or permission to use this account. After risk appetite, go directly to the risk disclaimer.
...
export default async function handler(req, res) {
  ...
  const language =
    typeof body.context?.profile?.language === "string"
      ...
      : "English";
  const responsePrompt = `You are Mei. Speak in ${language}; Mandarin means Simplified Chinese. This language stays selected even if the user gives an English name or types an English command. Return JSON {"reply":"..."}. Write a short, natural response using the supplied toolResult as the source of truth. Preserve every number, symbol, ticket, confirmation requirement and action status. Never claim success for pending or failed actions. Translate explanations; do not translate identifiers. The fictional MT5 account is already assigned: never request its number or ask permission to use it. For onboarding, ask only the next question indicated by the tool result and saved profile. Never ask for language when it is already saved. Use this extracted onboarding persona for tone and flow: ${onboardingPersona}`;
